...
Values makeValues(const bool isText,
    ...
    const double viewportZoomCorrection = pitchWithMap ? state.getSize().height / state.getViewport()[3]
                                                       : state.getViewport()[3] / state.getSize().height;
    return Values {
...
SymbolSDFProgram<Name, PaintProperties>::layoutUniformValues(const bool isText,
    ...
    const float gammaScale = values.pitchAlignment == AlignmentType::Map
                              ? std::cos(state.getPitch()) * state.getCameraToCenterDistance() * state.getViewport()[3] / state.getSize().height
                              : 1;
