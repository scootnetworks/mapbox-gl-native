...
void TransformState::getProjMatrix(mat4& projMatrix, uint16_t nearZ, bool aligned) const {
    ...
    const double viewportScale = getViewport()[2] / size.width;
    const double cameraToCenterDistanceAdjusted = getCameraToCenterDistance() * viewportScale;
    ...
    matrix::scale(projMatrix, projMatrix, 1.0, flippedY ? 1.0 : -1.0, viewportScale);
...
mat4 TransformState::getPixelMatrix() const {
    ...
    matrix::scale(m, m, viewport[2] / 2, -viewport[3] / 2, 1);
...
vec4 TransformState::getViewport() const {
    ...
    return {center.x - 0.5 * width, size.height - height - (size.height - center.y - 0.5 * height), width, height};
}
...
float TransformState::maxPitchScaleFactor() const {
    ...
    double viewportInherentScale = getViewport()[2] / size.width;
    return topPoint[3] / (getCameraToCenterDistance() * viewportInherentScale);
}
